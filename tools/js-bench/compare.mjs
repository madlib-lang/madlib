...
//
// Usage:
//   node tools/js-bench/compare.mjs
...
if (!existsSync(baselinePath)) {
  ...
  console.error("run: node tools/js-bench/run.mjs --baseline");
  process.exit(1);
}
if (!existsSync(resultsPath)) {
  console.error(`results.json not found at ${resultsPath}`);
  console.error("run: node tools/js-bench/run.mjs");
  process.exit(1);
}
...
process.stdout.write(`baseline: ${baseline.capturedAt}  (${baseline.node})\n`);
process.stdout.write(`current:  ${current.capturedAt}  (${current.node})\n\n`);
