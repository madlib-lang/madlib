...
import Explain.Format (prettyPrintType, predsToStr, schemeToStr, predToStr)
...
generateASTDoc depth (ast, docStrings) =
  ...
  in  "{\n"
    ...
    <> indent (depth + 1) <> "],\n"
    <> indent (depth + 1) <> "\"instances\": [\n"
    <> indent (depth + 2) <> generateInstancesDoc (depth + 2) docStrings instancesForDoc <> "\n"
    <> indent (depth + 1) <> "],\n"
...
generateADTDoc depth docStrings typeDecl = case typeDecl of
  Slv.Untyped _ (Slv.ADT name params ctors _ _) ->
    ...
    in  "{\n"
      <> indent (depth + 1) <> "\"type\": \"ADT\",\n"
      <> descriptionField
      <> exampleField
      <> sinceField
      <> indent (depth + 1) <> "\"name\": \"" <> name <> "\",\n"
      <> indent (depth + 1) <> "\"params\": \"" <> params' <> "\",\n"
      <> indent (depth + 1) <> "\"constructors\": [\n"
      <> indent (depth + 2) <> ctors'' <> "\n"
      <> indent (depth + 1) <> "]\n"
      <> indent depth <> "}"
