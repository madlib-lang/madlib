...
compileModule optimized coverage code = do
  let modulePath = "Module.mad"
  let options = buildOptions modulePath defaultPathUtils optimized coverage
  initialState <- Driver.initialState
  (jsCode, warnings, errors) <- Driver.runIncrementalTask
    initialState
    options
    [modulePath]
...
compileManyModulesWithReadFile pathUtils rootPath entrypoint modules = do
  let options = buildOptions entrypoint pathUtils False False
  initialState <- Driver.initialState
  let task = forM (M.keys modules) $ Rock.fetch . Query.GeneratedJSModule

  (builtModules, warnings, errors) <- Driver.runIncrementalTask
    initialState
    options { optRootPath = rootPath, optPathUtils = pathUtils }
    [entrypoint]
    modules
    Don'tPrune
    task

  unless (null errors) $ putStrLn (ppShow errors)

  return $ concat builtModules

compileManyModules :: FilePath -> M.Map FilePath String -> IO String
compileManyModules entrypoint modules = do
  let options = buildOptions entrypoint defaultPathUtils False False
  initialState <- Driver.initialState
  let task = forM (M.keys modules) $ Rock.fetch . Query.GeneratedJSModule
...
jsxProgram = unlines
  ...
  , "  inspect = (x) => #- new Integer(x).toString() -#"
  , "}"
  , ""
  , "interface Functor m {"
  , "  map :: (a -> b) -> m a -> m b"
  ...
  , "    <div>{`Current count is ${inspect(count)}`}</div>"
  , "    <div>"
  ...
  describe "compile" $ do
    ...
    it "should compile interfaces and instances" $ do
      let code = unlines
          ...
          actual = unsafePerformIO $ compileModule False False code
    ...
    it "should compile constrained instances and resolve their dictionary parameters" $ do
      let code = unlines
            ...
            , "  inspect = where { #[a, b, c] => \"#[\" ++ inspect(a) ++ \", \" ++ inspect(b) ++ \", \" ++ inspect(c) ++ \"]\" }"
            , "}"
            , ""
            , "instance (Inspect e, Inspect a) => Inspect (Either e a) {"
            , "  inspect = where {"
            , "    Right(a) => \"Right \" ++ inspect(a)"
            , "    Left(e) => \"Left \" ++ inspect(e)"
            , "  }"
            , "}"
            , ""
            , "fnWithConstraint :: Inspect a => a -> String"
            , "fnWithConstraint = inspect"
            , ""
            , "main = () => {"
            , "  inspect(#[1, false])"
            , "  inspect((Right(3) :: Either Integer Integer))"
            , "  inspect(#[1, 1])"
            , "  inspect(#[false, 42, true])"
            , "}"
            ]
          actual = unsafePerformIO $ compileModule False False code
    ...
    it "should compile monad transformers" $ do
      let actual = unsafePerformIO $ compileModule False False monadTransformersProgram
      snapshotTest "should compile monad transformers" actual

    it "should compile monad transformers with optimized dictionaries" $ do
      let actual = unsafePerformIO $ compileModule True False monadTransformersProgram
    ...
    it "should compile imports and exports" $ do
      let codeA  = "export singleton = (a) => ([a])"
          codeB  = unlines ["import L from \"./ModuleA\"", "import { singleton } from \"./ModuleA\"", "main = () => { L.singleton(3) }"]
          actual = unsafePerformIO $ compileManyModules "./ModuleB.mad" (M.fromList [("./ModuleA.mad", codeA), ("./ModuleB.mad", codeB)])
    ...
    it "should compile imports and exports of Namespaced ADTs" $ do
      let codeA = unlines
          ...
          actual = unsafePerformIO $ compileManyModules "./ModuleB.mad" (M.fromList [("./ADTs.mad", codeA), ("./ModuleB.mad", codeB)])
    ...
    it "should compile and resolve imported modules that import namespaced imports" $ do
      let
        ...
        pathUtils = defaultPathUtils { readFile = makeReadFile files, byteStringReadFile = makeByteStringReadFile files }

      let actual =
            unsafePerformIO $ compileManyModulesWithReadFile
              pathUtils
              "/root/project/src"
    ...
    it "should compile and resolve files importing prelude modules" $ do
      let listModule    = unlines ["map :: (a -> b) -> List a -> List b", "export map = (f, xs) => #- xs.map(f) -#"]
          ...
          main          = unlines ["import L from \"List\"", "main = () => { L.map((x) => (x * 2), [1, 2, 3]) }"]
          madlibDotJson = "{ \"main\": \"\" }"
