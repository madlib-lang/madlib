module Generate.Javascript.Internals where
...
eqFn target optimized =
  ...
  in  unlines
        ...
        , "      for (let i = 0; i < n; i++) {"
        , "        const k = '_' + i;"
        , "        if (!" <> fnName <> "(l[k], r[k])) return false;"
        , "      }"
        , "      return true;"
        , "    }"
        , ""
...
applyDictsFn target optimized =
  ...
  in  unlines
        ...
        -- Build the result object via mutable assignment instead of the
        -- previous `keys.reduce(spread)` chain — that chain allocated O(N)
        -- intermediate objects (one per key) for every dict it built.
