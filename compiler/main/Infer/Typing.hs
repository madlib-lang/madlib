...
updateTyping typing = case typing of
  Can.Canonical area (Can.TRSingle name   ) ->
    Slv.Untyped area $ Slv.TRSingle name

  Can.Canonical area (Can.TRComp name vars) ->
    Slv.Untyped area $ Slv.TRComp name (updateTyping <$> vars)

  Can.Canonical area (Can.TRArr  l    r   ) ->
  ...
  Can.Canonical area (Can.TRRowOverlay left right) ->
    Slv.Untyped area $ Slv.TRRowOverlay (updateTyping left) (updateTyping right)

  Can.Canonical area (Can.TRTuple elems) ->
    Slv.Untyped area $ Slv.TRTuple (updateTyping <$> elems)

  Can.Canonical area (Can.TRConstrained ts t) ->
    Slv.Untyped area $ Slv.TRConstrained (updateTyping <$> ts) (updateTyping t)
