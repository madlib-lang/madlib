...
rules options (Rock.Writer (Rock.Writer query)) = case query of
  ...
  SolvedASTWithEnv path -> nonInput $ do
    ...
    case res of
      Right (astAndEnv, InferState _ _ [] warnings _ _ _ _) -> do
        return (astAndEnv, (reverse warnings, mempty))

      Right ((ast, env), InferState _ _ errors warnings _ _ _ _) ->
...
runInfer a =
  runExceptT (runStateT a InferState { extensibleRecordsToDerive = mempty, count = 0, errors = [], Slv.warnings = [], mutatedNames = mempty, currentSubst = mempty, discardErrors = False, deltaSubst = mempty })
