...
tester code = buildAST "path" code >>= resolve initialEnv
  where initialEnv = Env M.empty M.empty

(Right astA) =
  buildAST "fixtures/sourceA.mad" $ unlines [ "import \"sourceB\""
                                            , "fn :: Num -> Num -> Num"
                                            , "fn = (a, b) => fn2(a, b) + a"
                                            ]

(Right astB) =
  buildAST "fixtures/sourceB.mad" $ unlines [ "fn2 :: Num -> Num -> Num"
                                            , "fn2 = (a, b) => a + b"
                                            ]
...
spec = do
  describe "compiler" $ do
    ...
    it "should resolve an AST table" $ do
      let
        astTable = M.fromList [("fixtures/sourceA.mad", astA), ("fixtures/sourceB.mad", astB)]
...
expected = M.fromList
  [ ( "fixtures/sourceA.mad"
    , AST
        { aimports =
            [ ImportDecl { ipos = Pos 0 1 1 , ipath = "sourceB" } ]
        , afunctions =
            [ FunctionDef
                { ftype = Just "Num"
                , ftypeDef =
                      ...
                      Operation
                        ...
                        , eleft =
                            FunctionCall
                              ...
                              , eargs =
                                  [ VarAccess
                                      { etype = Just "Num" , epos = Pos 60 3 20 , ename = "a" }
                                  , VarAccess
                                      { etype = Just "Num" , epos = Pos 63 3 23 , ename = "b" }
                                  ]
                              }
                        , eoperator = Plus
                        , eright =
                            VarAccess { etype = Just "Num" , epos = Pos 68 3 28 , ename = "a" }
                        }
                }
            ]
                ...
                , fbody =
                    Body
                      Operation
                        ...
                        , epos = Pos 41 2 17
                        , eleft =
                            VarAccess { etype = Just "Num" , epos = Pos 41 2 17 , ename = "a" }
                        , eoperator = Plus
                        , eright =
                            VarAccess { etype = Just "Num" , epos = Pos 45 2 21 , ename = "b" }
                        }
                }
            ]
